...
| `WalkSlow/Middle/Fast` | One `walking` state plus `walkSlow`, `walkMiddle`, `walkFast` flags | `WalkSlow`, `WalkMiddle`, `WalkFast` | Rust now owns explicit walk state identity from rollback-owned input facts. Walk entry uses extracted `PlCo.dat` `x24 == 23`; slow/middle/fast bucket cutoffs remain local input buckets while decomp-backed `x28`, `x2C`, and `x30` live separately as walk velocity ratios/taper data. Walk acceleration, target speed, and friction now follow `ftWalkCommon_800E0060` from deterministic Falcon profile/common data instead of local simulation constants. Walk exit through the `ft_8008A244`-shaped path now preserves carried `gr_vel`, which lets full-speed walk carry feed opposite-dash moonwalk setup instead of stopping first. |
| `Turn` | `turning` | `Turn` | Covered conceptually. Pygame mixes dash-out logic inside `turn()`. Rust now preserves carried `gr_vel` and applies source-shaped ground friction instead of zeroing velocity. |
| `TurnRun` | `runTurn` | `TurnRun` | Rust models the extracted PlCo `x38 == -48` Run-to-TurnRun threshold, old-facing `accel_mul`, same-frame source-shaped TurnRun physics after Run IASA entry, delayed facing flip at velocity crossing, jump-only IASA, and the extracted `x430` no-interrupt handoff back into Run. Exact animation command pause and completion timing remain partial. |
| `Dash` | `dashing` | `Dash` | Rust owns the decomp-shaped dash/tap core with extracted `PlCo.dat` deadzones (`x0`/`x4 == 36`), tap thresholds (`x8`/`xC == 32`), dash threshold (`x3C == 102`), tap window (`x40 == 2`), IASA windows (`x44`/`x48`/`x4C == 4/3/20`), `x54` Dash IASA velocity decay, source-shaped dash-entry velocity delta, next-frame live-stick Dash acceleration, tap-start early-Dash gating, Run-to-TurnRun threshold (`x38 == -48`), and run threshold (`x58 == 79`). Grounded horizontal movement now tracks decomp-shaped `gr_vel`, same-frame `xE4_ground_accel_1`, and post-translation Dash-entry `xE8_ground_accel_2` equivalents instead of directly overwriting horizontal velocity. Moonwalk-like movement remains an emergent Dash/tap-timer/live-stick outcome, including the bottom-gate payload `(+127,0) -> (-101,-45) -> (-101,-45) -> (-128,0)` and chained relay tests before Falcon's initial dash animation resolves. Non-run Dash completion now falls back to `Wait` with carried `gr_vel`; held-back follow-through is then handled by Wait's normal Turn/Walk priority instead of a direct Dash-to-Walk shortcut. |
...
| `RunBrake` | `endLag` flag while running | `RunBrake` | Rust has explicit RunBrake identity, source-shaped `gr_friction * x60` braking, jump/squat interrupts, and extracted Captain Falcon `max_run_brake_frames == 30`. Animation command-var pause behavior and the command-var-gated TurnRun branch remain partial; Pygame should stop using generic `endLag` for this. |
...
| `JumpF/JumpB` | Collapsed into `air` after jumpsquat | `JumpF`, `JumpB` | Rust is closer. Pygame cannot distinguish ground jump direction as a state. Profile-owned horizontal jump scaling now treats native stick `+127` as positive Melee `1.0` and `-128` as negative full-stick. Ground jump carry is now tested through the source `self_vel`/`gr_vel` handoff for walk, dash, Wait slide, and moonwalk follow-through: carried grounded speed receives jumpsquat friction through the prior grounded frame, is scaled by `ground_to_air_jump_momentum_multiplier`, and combines with held-stick `jump_h_initial_velocity` before the `jump_h_max_velocity` clamp. Rust also uses generated Captain Falcon action-frame counts to route `JumpF/B` animation completion into base `Fall`, matching `ftCo_Jump_Anim`. |
| `JumpAerialF/B` | Collapsed into `air` | `JumpAerialF`, `JumpAerialB` | Rust is closer. Profile-owned air-jump horizontal velocity uses the same asymmetric native-stick scale. The decomp also routes aerial-jump horizontal velocity through `init_h_vel`, `x74_anim_vel`, and `self_vel`, so full aerial parity is still source-audit work. |
...
| Aerial attacks | Five aerial states | Five aerial states plus `LandingAirN/F/B/Hi/Lw` | Aerial attack entry states are covered conceptually. Rust now enters exact `LandingAir*` states from aerial-attack ground contact, maps their generated Falcon ECB samples, and gates IASA with extracted Falcon `landingair*_lag` values. Stale landing-lag scaling and animation-rate scaling from `ftCo_LandingAir_EnterWithLag` remain pending. |
...
3. Rust derives a native `PADRead`/HSD-shaped pad sample: origin-subtracted
   triggers, origin-subtracted sticks, and HSD radius-clamped stick vectors.
...
3. Add tech/knockdown/passive states before building full combat.
